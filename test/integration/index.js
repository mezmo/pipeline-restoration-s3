...
const functionName = 'mezmo-pipeline-restoration'
...
test('integration tests', async (t) => {
  ...
  t.test('files found in root bucket', async (t) => {
    ...
    t.match(payload.StatusCode, 200)
    const res = JSON.parse(Buffer.from(payload.Payload).toString())
    t.match(res, {
      fileCount: 3
    ...
    if (sqsResult.Messages.length === 1) {
